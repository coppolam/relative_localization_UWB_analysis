...
elseif (strcmpi(behavior,'statfollower') && strcmpi(type,'acc'))    
    ...
    gamzoomlines = {'SE','SE';
        'NW','NW'};
elseif (strcmpi(behavior,'TowardsLeader') && strcmpi(type,'acchead'))    
    tticks = 0.5;
    tend = 50;
    pticks = 1;
    prange = [0,5];
...
elseif (strcmpi(behavior,"TowardsLeader"))
    p0l = [5;5];
    v0l = [-0.5;-0.5];
    a0l = [0;0];
    r0l = 0;
...
elseif (strcmpi(type,"acchead"))
    ...
    noisemeas = [0.2;0;0;0;0.1;0.1;0.1;0.1];
%     noisemeas = zeros(8,1);
    % obj.state = [x12;y12;z1;z2;u1;v1;u2;v2;gam];
    x0 = [0.1;0.05;0;0;vf(0);vl(0);1];
    P0 = diag([2^2 2^2 1^2 1^2 1^2 1^2 1^2 1^2 1^2]);
...
% plot(tarr,afarr(1,:));
% xlabel('time [s]');
% ylabel('ax follower [m/s2]');
% 
figure;
hold on
plot(tarr,prelreal(1,:));
plot(tarr,kalmanarr(1,:));
xlabel('time [s]');
ylabel('x12 [m]');
legend('true','kalman');
% 
figure;
hold on
plot(tarr,prelreal(2,:));
plot(tarr,kalmanarr(2,:));
xlabel('time [s]');
ylabel('y12 [m]');
legend('true','kalman');
...
figure;
hold on
plot(tarr,atand(prelreal(1,:)./prelreal(2,:)));
plot(tarr,atand(kalmanarr(1,:)./kalmanarr(2,:)));
xlabel('time [s]');
ylabel('bearing [deg]');
legend('true','kalman');
...
% figure;
% hold on
% plot(tarr,kalmanarr(5,:));
% plot(tarr,kalmanarr(7,:));
% xlabel('time [s]');
% ylabel('u [m/s]');
...
% figure;
% hold on
% plot(tarr,kalmanarr(6,:));
% plot(tarr,kalmanarr(8,:));
% xlabel('time [s]');
% ylabel('v [m/s]');
% legend('moving','static');
% 
...
% hold on
% plot(tarr,abserr(1,:));
% plot([tarr(1),tarr(end)],[MAEx,MAEx]);
% xlabel('time [s]');
% ylabel('x12 error [m]');
% legend('true','mean');
% 
...
% hold on
% plot(tarr,abserr(2,:));
% plot([tarr(1),tarr(end)],[MAEy,MAEy]);
% xlabel('time [s]');
% ylabel('y12 error [m]');
% legend('true','mean');
% 
% figure;
% hold on
% plot(tarr,abserrtotal);
% plot([tarr(1),tarr(end)],[MAEtotal,MAEtotal]);
% xlabel('time [s]');
...
%     set(h,'Visible','off');
% end
% % set(gca,'XTick',1:tend,'FontSize', fontsize,'FontUnits','points','FontWeight','normal','FontName','Times');
% hold on
% grid on
% plot(tarr,abserr(1,:),'linewidth',linewidth);
% % axis([0 tend -Inf Inf]);
% xlabel('time [s]','FontUnits','points','interpreter','latex','FontSize',fontsize,'FontName','Times');
% ylabel('$x$ error [m]','FontUnits','points','interpreter','latex','FontWeight','normal','FontSize',fontsize,'FontName','Times');
% file = strcat('C:\Users\Steven\Dropbox\Thesis\Figures\Observability_kinematic\',filename,'_x.eps');
% if (printfigs)
%     print(file,'-depsc2');
% end
% 
...
% hold on
% grid on
% plot(tarr,abserr(2,:),'linewidth',linewidth);
% % axis([0 tend -Inf Inf]);
% xlabel('time [s]','FontUnits','points','interpreter','latex','FontSize',fontsize,'FontName','Times');
% ylabel('$y$ error [m]','FontUnits','points','interpreter','latex','FontWeight','normal','FontSize',fontsize,'FontName','Times');
% file = strcat('C:\Users\Steven\Dropbox\Thesis\Figures\Observability_kinematic\',filename,'_y.eps');
% if (printfigs)
%     print(file,'-depsc2');
% end
...
if(pzoom)
    ax2= MagInset(h,-1,pzoomarea,pzoomput,pzoomlines);
    set(ax2,'FontSize',fontsize2);
    set(ax2,'YTickLabel',[]);
    set(ax2,'XTickLabel',[]);
...
if(gamzoom)
    ax2= MagInset(h,-1,gamzoomarea,gamzoomput,gamzoomlines);
    set(ax2,'FontSize',fontsize2);
    set(ax2,'YTickLabel',[]);
    set(ax2,'XTickLabel',[]);
    % set(ax2,'XTick',pxticks2);
