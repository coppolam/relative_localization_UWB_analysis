...
%%
rangenoisearr = [0 0.1 0.25 0.5];
storeres = cell(1,numel(rangenoisearr));
storestd = cell(1,numel(rangenoisearr));
indres   = cell(1,numel(rangenoisearr));

for jjj = 1:numel(rangenoisearr)
    clearvars -except jjj rangenoisearr indres storeres storestd
    
    compahead = true;
    typearr       = 2;
    rangenoise    = rangenoisearr(jjj);      % [0 0.1 0.25 0.5 1 2 4 8];
    angles        = 0 : 1/16*pi : 2*pi; % 0 parallel, pi/2 not parallel
    scalearr      = 1;
    if rangenoisearr(jjj) == 0
        nruns = 1;
    else
        nruns = 100;
    end
    ...
    % Store Matrices
    storeres{jjj} = zeros(length(typearr)*length(angles),length(scalearr));
    storestd{jjj} = zeros(length(typearr)*length(angles),length(scalearr));
    indres{jjj}   = zeros(length(typearr)*length(angles),length(scalearr),nruns);
...
%                                     keyboard
                ...
                indres  {jjj}((typeindex-1)*(length(angles))+headingindex,scaleindex,:) = avgerr;
                storeres{jjj}((typeindex-1)*(length(angles))+headingindex,scaleindex  ) = mean(avgerr);
                storestd{jjj}((typeindex-1)*(length(angles))+headingindex,scaleindex  ) = std(avgerr);
...
save('storeresmat_anglestuff','storeres','storestd');
%%
% condmatrixscript
set(0, 'defaulttextinterpreter', 'latex');
h = newfigure(242);
for i = 1:numel(storeres)
    plot(angles,storeres{i},'o-'); hold on;
end
xlabel('Angle between velocity vectors [rad]')
ylabel('AMAE distance from truth [m]')
legend('$\sigma_R$=0 m','$\sigma_R$=0.1 m','$\sigma_R$=0.25 m','$\sigma_R$=0.5 m')
makeaxespi(h,[0 2*pi],'x')
latex_printallfigures(get(0,'Children'), '','paper_wide_half',242)

%%
% close all
% figure(1)
% surf(scalearr,angles,storeres{1});
% xlabel('Speed Scale')
...
% toc
% save('storeresmat_teststuff','storeres','storestd');
