function latex_maketable(matrix, filename, varargin)
...
% Enjoy life!!!
    ...
    okargs = {'rowlabels','columnlabels', 'alignment', 'format', 'size','topcornerlabel'};
    for j=1:2:(nargin-2)
        pname = varargin{j};
        pval = varargin{j+1};
        ...
        elseif length(k)>1
            ...
            switch(k)
                ...
                case 5  % format
                    textsize = pval;
                case 6  % topcornerlabel
                    topCornerLabel = pval;
                    if isnumeric(topCornerLabel)
                        topCornerLabel = cellstr(num2str(topCornerLabel(:)));
                    end
            end
        end
    end
    ...
    if(~isempty(topCornerLabel))
        fprintf(fid,['\\textbf{',topCornerLabel, '} ']);
    end
