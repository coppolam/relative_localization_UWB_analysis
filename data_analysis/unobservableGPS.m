...
datapath = '../../../Data/NDI/01-02-2018/GPS';
addpath(datapath);
...
file = strcat('C:\Users\Steven\Dropbox\Thesis\Figures\NDI_experiments\unobsGPS2tarr.eps');
...
file = strcat('C:\Users\Steven\Dropbox\Thesis\Figures\NDI_experiments\dtcorr.eps');
tcompstart = 130;
tcompend = 136;
...
% yyaxis right
% plot(newtshift(newtshifttm),dtarrtmp(newtshifttm),'--','linewidth',linewidth,'color',right_color);
% ylabel('dt [s]');
% xlabel('time [s]');
% % leg1= legend('localization error','dt');
...
file = strcat('C:\Users\Steven\Dropbox\Thesis\Figures\NDI_experiments\unobscorr.eps');
h = figure;
if(printfigs)
    set(h,'visible','off');
end
...
xlabel('time [s]','FontUnits','points','interpreter','latex','FontSize',fontsize,'FontName','Times');
% leg1= legend('Localization error','Unobservable points','location','bestoutside');
% set(leg1,'interpreter','latex');
if(printfigs)
    print(h,file,'-depsc2');
end
% figure;
% hold on;
% plot(newt1,plocerr,'linewidth',linewidth);
% yyaxis left
% plot(newt1,-1*(binvalarr-1),'--','linewidth',linewidth,'color',right_color);
% 
% figure;
% hold on;
% plot(newt,binvalarr);
...
xt = 20;
yax = [0 500];
file = strcat('C:\Users\Steven\Dropbox\Thesis\Figures\NDI_experiments\unobsGPS2hist.eps');
fontsize = 26;
h = figure;
if(printfigs)
    set(h,'visible','off');
...
% figure;
% hold on;
% histogram(valvelarr,nbins);
% xlabel('value');
% ylabel('instances');
% 
% [rho,p] = corr(valarr.',valvelarr.');
